...
//                self.nextBtn.alpha = 1.0
    ...
    func initialSetup()
    {
        ...
        self.emailView.applyGradient(colours:PRIMARY_COLOR)
    ...
    func checkexistingemailAPI()
    {
        ...
        apollo.fetch(query: checkemail,cachePolicy:.fetchIgnoringCacheData){ (result,error) in
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.0, execute: {
               ...
               // self.navigationController?.pushViewController(passwordObj, animated: true)
            })
            }
            else{
                self.lottieView.isHidden = true
...
//        }else {
//
//          //  self.view.makeToast("No special Characters allowed")
//            self.nextBtn.isUserInteractionEnabled = false
//            self.nextBtn.alpha = 0.7
//        }
//
    ...
    func textFieldDidEndEditing(_ textField: UITextField) {
        if textField == emailTF{
            if !(emailTF.text?.isValidEmail())!{
                self.view.makeToast("\((Utility.shared.getLanguage()?.value(forKey:"email_validalert"))!)")
              //  bottomView.makeToast("No special Characters allowed")
            }
        }
    }
}
